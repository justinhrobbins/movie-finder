...
export const ActortAlertSummaryCard = ({ userActorAlerts}) => {
    ...
    return (
        <div className="ActortAlertSummaryCard">
            <div className="actor-alert-data-card-total-actors"><Link className="actor-detail-alert-data-card-link" to={`/actoralerts?sort=name&filter=all`} title="Show all movies">You are following {userActorAlerts.actorCount} Actors</Link></div>
